...
3. **接口校验与限频**  
   `/sendNotification` 仅允许 `POST`，并校验同源请求（`Origin` 与 Worker 域名一致）。同时做了简单限频：单个 IP 60 秒内最多 3 次请求。

4. **参数校验与 UID 解析**  
   发送通知前会校验 `WXAPPTOKEN` 和 `WXPUSHER_UIDS` 是否配置；`WXPUSHER_UIDS` 支持逗号分隔字符串（自动 trim）。

5. **交互优化**  
   通知发送成功才进入 60 秒倒计时；失败会立即恢复按钮。未配置电话号码时拨号按钮会提示。

### 备注
- 使用环境变量获取参数，不能使用 JS 加密。
- 限频为内存级简单策略，Worker 冷启动或实例扩容时会重置计数。
